...
def test_add_pets_with_valid_data(name='Fedot', animal_type='cat', age='3', pet_photo='images/Fedot.jpg'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pets(api_key, 'Муся', 'кошка', '2', 'images/Муся.jpg')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_update_pet_info(name='New', animal_type='New-cat', age='10'):
...
def test_add_pets_with_valid_data_without_photo(name='Буся', animal_type='кот', age='2'):
...
def test_successful_update_self_pet_info(name='Муся', animal_type='Кошка', age='2'):
...
def test_add_pet_negative_age_number(name='Ася', animal_type='cat', age='-3', pet_photo='images/Fedot.jpg'):
...
def test_add_pet_with_four_digit_age_number(name='Fedot', animal_type='cat', age='1234', pet_photo='images/Fedot.jpg'):
...
def test_add_pet_with_empty_value_in_variable_name(name='', animal_type='cat', age='2', pet_photo='images/Буся.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='cat', age='2', pet_photo='images/Fedor.jpg'):
    '''Проверка с негативным сценарием. Добавления питомца имя которого превышает 3 слова
    Тест не будет пройден если питомец будет добавлен на сайт с именем состоящим из более 3 слов'''

    name = 'Кот по имени Федот'
    ...
    assert status == 200
    assert word_count < 3, 'Питомец добавлен с именем больше 3 слов'


def test_add_pet_with_special_characters_in_variable_animal_type(name='Fedot', age='3', pet_photo='images/Fedot.jpg'):
...
def test_add_pet_with_numbers_in_variable_animal_type(name='Буся', animal_type='34562', age='3',
                                                      pet_photo='images/Буся.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_animal_type(name='Fedot', age='3', pet_photo='images/Fedot.jpg'):
    '''Проверка с негативным сценарием. Добавления питомца название породы которого превышает 3 слова
    Тест не будет пройден если питомец будет добавлен на сайт с названием породы состоящим более 3 слов'''

    animal_type = 'Кот белый пушистый беспородный'
    ...
    assert status == 200
    assert word_count < 3, 'Питомец добавлен с названием породы больше 3 слов'
...
def test_get_api_key_with_wrong_email_and_correct_password(email=invalid_email, password=valid_password):
    '''Проверяем запрос с валидным паролем и невалидным емейлом.
    Проверяем нет ли ключа в ответе'''
    status, result = pf.get_app_key(email, password)
    assert status == 403
    assert 'key' not in result
